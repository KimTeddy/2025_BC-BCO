...
def train(env, bc_lunarlander, training_set, testing_set, criterion):
    ...
    n_epoch = 50
    learning_rate = 0.001
    optimizer = torch.optim.Adam(bc_lunarlander.parameters(), lr = learning_rate) 
    for itr in range(n_epoch):
        ...
        y_pred = bc_lunarlander(x)
        test_loss.append(criterion(y_pred, y).item())

    # plot test loss
    torch.save(bc_lunarlander, "saves/bc_lunarlander_n=2") # uncomment to save the model
...
def test(bc_lunarlander, testing_set, criterion):
    ...
    print(bc_lunarlander(testing_set[p, :state_space_size]) )
    print(testing_set[p, state_space_size:])
    criterion( bc_lunarlander(testing_set[p, :state_space_size] ), testing_set[p, state_space_size:] ).item()
    ...
    ################################## interact with env ##################################
        ...
        for ep in range (n_ep):
            ...
            for t in range (max_steps):      
                # action = bc_lunarlander(torch.tensor(state, dtype=torch.float))
                # action = np.clip(action.detach().numpy(), -2,2)
                with torch.no_grad():
                    logits = bc_lunarlander(torch.tensor(state, dtype=torch.float).unsqueeze(0))
...
def main():
    ...
    train(env, bc_lunarlander, training_set, testing_set, criterion)
    seed_reward_mean = test(bc_lunarlander, testing_set, criterion)
