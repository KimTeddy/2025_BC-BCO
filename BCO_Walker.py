...
def train_bco(bco_walker, training_set,  criterion, state_space_size, testing_set):
...
def train_bco_environmet(env, state_trainsition_model, expert_states, bco_walker):
    ...
    ################################## interact with env ##################################
        ...
        if np.mean(seed_reward_mean[-1]) > target_reward:
            torch.save(bco_walker, "saves/bco_walker={}_BCO({})_best_{}_expert_states".format(n,itr,expert_states.shape[0]))        
            break
        ...
        bco_walker = train_BC(state_space_size, expert_data, bco_walker, n_epoch =100)
...
def test_bco_environmet(env, bco_walker):
    ...
    ################################## interact with env ##################################
        ...
        print("Itr = {} overall reward  = {:.6f} ".format(itr, np.mean(seed_reward_mean[-1])))
        print("Interacting with environment finished")
    # np.save("reward_mean_walker_n={}_bco({})_expert_states={}".format(n, x , expert_states.shape[0]), seed_reward_mean) #uncomment to save the reward over 5 random seeds
...
def main():
    ...
    states_new_agent  =  torch.tensor(np.load ("data/states_walker_exploration.npy")[:1000], dtype= torch.float)
    actions_new_agent =  torch.tensor(np.load ("data/actions_walker_exploration.npy")[:1000], dtype= torch.float)
...
# Load pre-trained Transition Model and predict infered expert actions from expert states only-------------
    ...
    train_bco(bco_walker, training_set, criterion, state_space_size, testing_set)
    train_bco_environmet(env, state_trainsition_model, expert_states, bco_walker)
    seed_reward_mean = test_bco_environmet(env, bco_walker)
...
# Expert
    expert  = np.load("reward_mean_walker_expert.npy")
