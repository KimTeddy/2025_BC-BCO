...
def train(env, bc_pusher, training_set, testing_set, criterion):
    # init environment
    ...
    n_epoch = 50
    learning_rate = 0.001
    optimizer = torch.optim.Adam(bc_pusher.parameters(), lr = learning_rate) 
    ...
    # plot test loss
    torch.save(bc_pusher, "saves/bc_pusher_n=2") # uncomment to save the model
...
def test(bc_pusher, testing_set, criterion):
    ...
    print(bc_pusher(testing_set[p, :state_space_size]) )
    print(testing_set[p, state_space_size:])
    criterion( bc_pusher(testing_set[p, :state_space_size] ), testing_set[p, state_space_size:] ).item()
    ...
    ################################## interact with env ##################################
        ...
        for ep in range (n_ep):
            ...
            for t in range (max_steps):      
                action = bc_pusher(torch.tensor(state, dtype=torch.float))
...
def main():
    ...
    train(env, bc_pusher, training_set, testing_set, criterion)
    seed_reward_mean = test(bc_pusher, testing_set, criterion)
